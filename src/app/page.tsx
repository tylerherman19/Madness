...
export default async function Home(){
 ...
 <main className={s.main}>
  ...
  <dl className={s.stats}><div><dt>Still alive</dt><dd>{data?.aliveCount??'â'}<small>{data?.totalPlayers?Math.round((data.aliveCount/data.totalPlayers)*100):0}% of field</small></dd></div><div><dt>Eliminated</dt><dd>{data?.eliminatedCount??'â'}<small>{data?.totalPlayers??0} total entries</small></dd></div><div><dt>Current pot</dt><dd>{data?'$'+data.potSize.toLocaleString():'â'}<small>Last one wins</small></dd></div><div><dt>Picks in</dt><dd>{data?.picksMade??'â'}<small>of {data?.aliveCount??'â'} survivors</small></dd></div></dl>
