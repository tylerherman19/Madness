...
import { formatPeriodDate, roundOf64Countdown } from '@/lib/competition'
import { haveSignupsClosed } from '@/lib/season'
import { getEffectiveNow } from '@/lib/testMode'
import { fetchDayScoreboard } from '@/lib/espn'
export const revalidate=60
const CT='America/Chicago'
export default async function Home(){
 const [data,closed,now]=await Promise.all([getDashboardData(),haveSignupsClosed(),getEffectiveNow()])
 // The countdown the pool is actually waiting on: the tournament's first full
 // round. Recomputed on every revalidate, so the day count never goes stale.
 const round64=roundOf64Countdown(now,data?.slate?.season_year)
 ...
 <dl className={s.stats}><div><dt>Prize pool</dt><dd>{data?'$'+data.potSize.toLocaleString():'—'}<small>{data?.totalPlayers??'—'} entries</small></dd></div><div><dt>Still standing</dt><dd>{data?.aliveCount??'—'}<small>{data?.eliminatedCount??'—'} eliminated</small></dd></div><div><dt>{round64.days===0?'Round of 64':'Round of 64 begins in'}</dt><dd>{round64.days===0?'Today':round64.days.toLocaleString()}<small>{round64.days===0?round64.dateLabel:(round64.days===1?'day · ':'days · ')+round64.dateLabel}</small></dd></div><div><dt>Picks submitted</dt><dd>{data?.picksMade??'—'}<small>of {data?.aliveCount??'—'} survivors</small></dd></div></dl>
