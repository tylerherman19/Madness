...
export default async function Home(){
 ...
 <main className={s.main}>
  ...
  <div className={s.dashboardGrid}>
   ...
   <section className={s.gamesCard}><div className={s.sectionTitle}><h2>Today&apos;s games</h2><Link href="/schedule">View all games <Arrow/></Link></div><div className={s.gameTable}><div className={s.gameHead}><span>Time</span><span>Matchup</span><span>Status</span></div>{games.slice(0,5).map(g=><Link href="/schedule" className={s.gameRow} key={g.id}><time>{g.time_tbd?'TBD':tip(g.tip_time)}</time><span className={s.gameTeams}><i><Logo team={g.away_team} brands={brands} size={25}/>{brandFor(g.away_team,brands).shortName}</i><small>vs</small><i><Logo team={g.home_team} brands={brands} size={25}/>{brandFor(g.home_team,brands).shortName}</i></span><b>{g.status_state==='post'?'Final':g.status_state==='in'?'Live':'Upcoming'}</b></Link>)}{games.length===0&&<p className={s.empty}>The next slate will appear here when it is set.</p>}</div></section>
  </div>
