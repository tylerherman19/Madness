...
export default function ScheduleBoard({days,season,brands}:{days:ScheduleDay[];season:number;brands:TeamBrandDirectory}){
 ...
 return <><div className={s.pageHeading}><div><p className={s.context}>College basketball Â· {season} season</p><h1>Every game, at a glance.</h1><p>Scores, the next slate, and your path through the season.</p></div><Link href="/pick" className={s.secondary}>Make daily pick <Arrow/></Link></div>
 ...
 return <article className={s.scoreCard} key={game.awayAbbr+game.homeAbbr+index}><div className={s.scoreMeta}><span>{result&&result.state!=='pre'?result.statusText:game.timeTbd?'Time TBD':new Date(game.kickoff).toLocaleTimeString('en-US',{timeZone:'America/Chicago',hour:'numeric',minute:'2-digit'})+' CT'}</span><small>{game.tv}</small></div>{[game.awayAbbr,game.homeAbbr].map((team,i)=><div className={s.scoreTeam} key={team}><Logo team={team} brands={brands} size={38}/><div><strong>{name(team)}</strong><small>{game.round?[(i===0?game.awaySeed:game.homeSeed)&&'Seed '+(i===0?game.awaySeed:game.homeSeed),game.region].filter(Boolean).join(' Â· '):i===0?'Away':'Home'}</small></div><b>{result&&result.state!=='pre'&&result.scoresKnown!==false?(i===0?result.awayScore:result.homeScore):game.odds?american(i===0?game.odds.awayMoneyline:game.odds.homeMoneyline):'â'}</b></div>)}<p>{game.odds?`Spread ${point(game.odds.spreadHome)} Â· O/U ${game.odds.total??'â'} Â· ${game.odds.bookmaker}`:(game.round??'Odds not posted yet')} Â· All times Central</p></article>
 })}</div>}<p className={s.tableNote}>Live scores refresh automatically when available. Odds are supplied by The Odds API when bookmakers post them. Unannounced tip times remain TBD.</p></>
}
